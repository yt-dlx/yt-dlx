   ...
   <motion.section className="flex items-center justify-center border-b-8 border-double border-[#cd322d6e] w-full py-12 md:py-24 lg:py-32 bg-black text-white">
    <div className="container px-4 md:px-6">
     <form
      ...
      <motion.h2 className="text-3xl font-black text-red-600 mb-4" {...FromBottomToTop}>
       Effortless Audio Video Downloader And Streamer!
      </motion.h2>
      <motion.p className="mb-8 italic text-white font-semibold" {...FromBottomToTop}>
       Unlock the power of YT-DLX, the ultimate node.js toolkit for seamless audio and video downloading and streaming. Effortlessly handle various coding flavors, from TypeScript to CommonJS and ESM,
       ensuring 100% compatibility and comprehensive type safety.
      </motion.p>
      <div className="flex items-center gap-4">
       <label className="form-control w-full">
        <div className="label">
         <span className="flex flex-row items-center justify-center label-text text-red-600 text-sm font-black">
         ...
         <span className="flex flex-row items-center justify-center label-text-alt text-red-600 text-sm font-black">
          <HiFire size={20} className="text-red-600 animate-pulse" />
          option:
          ...
          {TubeSearch &&
           TubeSearch.map((item: any, index: number) => (
            <Link
             key={index}
             href={`/gui/${item.id}`}
             className="relative group mb-4 duration-700 bg-[#111111] p-1 shadow-red-600 hover:shadow-[0_0_400px_rgba(255,0,0,0.5)] rounded-3xl border-2 border-red-600/40 hover:border-red-600 flex flex-col justify-between">
             <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-700">
               <SiGradleplaypublisher className="text-red-600 animate-spin" size={140} />
              </div>
             </div>
             <div className="flex-1 flex flex-col p-2">
              <p className="mb-2 text-xs font-semibold tracking-wider text-red-600 uppercase">number :: {index}</p>
              <motion.h2 className="mb-2 text-xl font-black leading-snug text-red-600" {...FromBottomToTop}>
               {item.title}
              </motion.h2>
              ...
              <div className="flex items-center text-white">
               <ul className="mb-3 list-disc ml-4 text-xs">
                ...
                <li>
                 <span className="text-red-600 text-sm">channelid:</span> {item.channelid}
                </li>
                <li>
                 <span className="text-red-600 text-sm">channelname:</span> {item.channelname}
                </li>
                <li>
                 <span className="text-red-600 text-sm">duration:</span> {item.duration}
                </li>
                <li>
                 <span className="text-red-600 text-sm">uploadDate:</span> {item.uploadDate}
                </li>
                <li>
                 <span className="text-red-600 text-sm">viewCount:</span> {item.viewCount}
                </li>
               </ul>
              </div>
             </div>
