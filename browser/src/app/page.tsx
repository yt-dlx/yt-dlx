...
export default function Home() {
  ...
  const handleVideoSelect = async (video: SearchResult) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`/api/youtube?action=AudioVideoHighestData&query=${encodeURIComponent(video.id)}`);
      const rawText = await response.text();
      const data: any = JSON.parse(rawText);
      if (!response.ok || data.error) throw new Error(data.error || "Failed to fetch video data.");
      const VideoHighFUrl = data.VideoHighF.url;
      const AudioHighFUrl = data.AudioHighF.url;
      if (VideoHighFUrl) setVideoUrl(VideoHighFUrl);
      else throw new Error("No playable video URL found.");
      if (AudioHighFUrl) setAudioUrl(AudioHighFUrl);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Something went wrong. Please try again.");
      setVideoUrl(null);
      setAudioUrl(null);
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-[#f1f1f1] px-4 sm:px-6 lg:px-8 flex flex-col justify-between">
        ...
        {selectedVideo && (
          <motion.div
            ...
            <motion.div
              ...
              exit={{ scale: 0.7, y: 50 }}
              transition={{ duration: 0.3 }}>
              <button className="absolute top-2 right-2 text-gray-600 hover:text-red-600 text-2xl" onClick={closeVideoPlayer}>
                ×
              </button>
              <div className="w-full">
                {videoUrl && (
                  <ReactPlayer
                    ...
                    onSeek={handleSeek} // Sync both on seek
                  />
                )}
                {audioUrl && (
                  <ReactPlayer
                    ...
                    height="auto"
                    playing={true}
                    onError={e => setError(`Audio playback error: ${e}`)}
                    onSeek={handleSeek} // Sync both on seek
                  />
                )}
                {!videoUrl && !audioUrl && <p className="text-red-600 text-center">Loading video... {error}</p>}
              </div>
              <div className="mt-4">
                ...
                <div className="flex items-center mt-2">
                  <div className="w-10 h-10 rounded-full overflow-hidden mr-2">
                    {selectedVideo.thumbnails && selectedVideo.thumbnails.length > 0 && (
                      <Image src={selectedVideo.thumbnails[0].url} alt={selectedVideo.channelname || "Channel"} width={40} height={40} className="object-cover" />
                    )}
                  </div>
                  <div>
      ...
      <motion.footer className="bg-[#212121] w-full py-3 fixed bottom-0 left-0 flex justify-around items-center shadow-lg" initial={{ y: 100 }} animate={{ y: 0 }} transition={{ duration: 0.5 }}>
        <button className="text-[#aaaaaa] hover:text-[#ff0000] text-center flex-1 py-2" onClick={handleSearch}>
