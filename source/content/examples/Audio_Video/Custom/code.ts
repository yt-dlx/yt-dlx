...
YouTubeDLX.Audio_Video.Custom({ resolution: "720p" } as any).on("error", error => console.error("Expected Error (missing query):", error));

// 18. Missing required 'resolution' parameter (will result in an error)
YouTubeDLX.Audio_Video.Custom({ query: "your search query or url" } as any).on("error", error => console.error("Expected Error (missing resolution):", error));

// 19. Invalid 'resolution' value (will result in an error - Zod validation)
YouTubeDLX.Audio_Video.Custom({ query: "your search query or url", resolution: "500p" as any }).on("error", error => console.error("Expected Error (invalid resolution):", error));

// 20. Invalid 'filter' value (will result in an error - Zod validation)
YouTubeDLX.Audio_Video.Custom({ query: "your search query or url", resolution: "720p", filter: "nonexistentfilter" as any }).on("error", error =>
  console.error("Expected Error (invalid filter):", error),
);
