...
YouTubeDLX.Audio.Lowest({} as any).on("error", error => console.error("Expected Error (missing query):", error));

// 18. Invalid 'filter' value (will result in an error - Zod validation)
YouTubeDLX.Audio.Lowest({ query: "your search query or url", filter: "nonexistentfilter" as any }).on("error", error => console.error("Expected Error (invalid filter):", error));
