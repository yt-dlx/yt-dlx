...
YouTubeDLX.Audio.Custom({ query: "your search query or url", resolution: "high", metadata: true, output: "./should_fail" }).on("error", error =>
  console.error("Expected Error (output with metadata):", error),
);

// 14. Attempt to use stream with metadata (will result in an error)
YouTubeDLX.Audio.Custom({ query: "your search query or url", resolution: "high", metadata: true, stream: true }).on("error", error => console.error("Expected Error (stream with metadata):", error));

// 15. Attempt to use filter with metadata (will result in an error)
YouTubeDLX.Audio.Custom({ query: "your search query or url", resolution: "high", metadata: true, filter: "speed" }).on("error", error =>
...
YouTubeDLX.Audio.Custom({ resolution: "high" } as any).on("error", error => console.error("Expected Error (missing query):", error));

// 18. Missing required 'resolution' parameter (will result in an error - Zod validation)
YouTubeDLX.Audio.Custom({ query: "your search query or url" } as any).on("error", error => console.error("Expected Error (missing resolution):", error));

// 19. Invalid 'resolution' value (will result in an error - Zod validation)
YouTubeDLX.Audio.Custom({ query: "your search query or url", resolution: "superhigh" as any }).on("error", error => console.error("Expected Error (invalid resolution):", error));

// 20. Invalid 'filter' value (will result in an error - Zod validation)
YouTubeDLX.Audio.Custom({ query: "your search query or url", resolution: "high", filter: "nonexistentfilter" as any }).on("error", error => console.error("Expected Error (invalid filter):", error));
...
YouTubeDLX.Audio.Custom({ query: "a query that should return no results 12345abcde", resolution: "high" }).on("error", error => console.error("Expected Error (no engine data):", error));
...
YouTubeDLX.Audio.Custom({ query: "some video with limited formats", resolution: "ultralow" }).on("error", error => console.error("Expected Error (no audio data for resolution):", error));
