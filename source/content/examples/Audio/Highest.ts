...
YouTubeDLX.Audio.Highest({ query: "your search query or url", metadata: true, filter: "speed" }).on("error", error => console.error("Expected Error (filter with metadata):", error));
...
YouTubeDLX.Audio.Highest({} as any).on("error", error => console.error("Expected Error (missing query):", error));

// 18. Invalid 'filter' value (will result in an error - Zod validation)
YouTubeDLX.Audio.Highest({ query: "your search query or url", filter: "nonexistentfilter" as any }).on("error", error => console.error("Expected Error (invalid filter):", error));
