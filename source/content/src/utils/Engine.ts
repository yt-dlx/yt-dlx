...
// if (verbose) console.log(colors.green("@info:"), `Attempting to spawn Tor at: ${torPath}`);
...
// });
// if (verbose) console.log(colors.green("@info:"), `Spawned Tor process with PID: ${torProcess.pid} using ${torPath}. Waiting for bootstrap...`);
// });
// };
...
export default async function Engine({ query, useTor = false, verbose = false }) {
  ...
  if (useTor) {
    ytprobeArgs.unshift("--proxy", "socks5://127.0.0.1:9050");
    if (verbose) console.log(colors.green("@info:"), "Using Tor proxy for ytprobe request");
  }
