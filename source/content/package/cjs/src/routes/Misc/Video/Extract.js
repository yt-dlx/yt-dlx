...
async function fetchVideoTranscript(videoId, verbose) {
    try {
        if (verbose)
            console.log(colors_1.default.green("@info:"), `Fetching transcript for video ID: ${videoId}`);
...
function extract(options) {
    ...
    (async () => {
        try {
            ...
            if (!metaBody) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Unable to get response!`);
                return;
            }
            if (!metaBody.metaData) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Metadata not found in the response!`);
                return;
            }
            let uploadDate;
            try {
                ...
                emitter.emit("error", `${colors_1.default.red("@error:")} Failed to parse upload date: ${error instanceof Error ? error.message : String(error)}`);
            }
