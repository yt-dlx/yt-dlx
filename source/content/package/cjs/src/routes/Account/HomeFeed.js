...
function home_feed(options) {
    ...
    (async () => {
        try {
            ...
            if (verbose)
                console.log(colors_1.default.green("@info:"), "Fetching home feed...");
            if (!cookies) {
            ...
            if (!homeFeed) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Failed to fetch home feed.`);
                return;
            }
            ...
            homeFeed.contents?.contents?.forEach((section) => {
                if (section?.type === "RichItem" && section?.content?.type === "Video") {
                    const sanitized = (0, sanitizeContentItem_1.default)(section);
            ...
            if (verbose)
                console.log(colors_1.default.green("@info:"), "Home feed fetched!");
