...
const ZodSchema = zod_1.z.object({
    ...
    minViews: zod_1.z.number().optional(),
    maxViews: zod_1.z.number().optional(),
    verbose: zod_1.z.boolean().optional(),
    orderBy: zod_1.z.enum(["relevance", "viewCount", "rating", "date"]).optional(),
});
/**
 ...
 *
 */
function search_videos({ query, minViews, maxViews, orderBy, verbose }) {
    ...
    (async () => {
        try {
            ...
            if (videos.length === 0) {
                emitter.emit("error", `${colors_1.default.red("@error: ")} No videos found with the given criteria.`);
                return;
            }
