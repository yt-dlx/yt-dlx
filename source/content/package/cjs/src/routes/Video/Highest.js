 ...
 *
 */
function VideoHighest({ query, stream, verbose, output, metadata, useTor, filter }) {
    ...
    (async () => {
        try {
            ...
            if (!engineData.VideoHighF?.url) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Highest quality video URL not found.`);
                return;
            }
