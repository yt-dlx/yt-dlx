...
function AudioCustom({ query, output, useTor, stream, filter, verbose, metadata, resolution }) {
    ...
    (async () => {
        try {
            if (!query) {
                emitter.emit("error", `${colors_1.default.red("@error:")} The 'query' parameter is required.`);
                return;
            }
            if (metadata) {
                if (stream) {
            ...
            if (!resolution) {
                emitter.emit("error", `${colors_1.default.red("@error:")} The 'resolution' parameter is required.`);
                return;
            }
            ...
            if (!engineData.metaData) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Metadata was not found in the engine's response.`);
                return;
            }
            if (metadata) {
            ...
            if (!adata) {
                emitter.emit("error", `${colors_1.default.red("@error:")} No audio data found for the specified resolution: ${resolution}. Please use the 'list_formats()' command to see available formats.`);
                return;
            }
            if (!engineData.metaData.thumbnail) {
                emitter.emit("error", `${colors_1.default.red("@error:")} The thumbnail URL was not found.`);
                return;
            }
            instance.addInput(engineData.metaData.thumbnail);
            instance.withOutputFormat("avi");
            if (!adata.url) {
                emitter.emit("error", `${colors_1.default.red("@error:")} The audio URL was not found.`);
                return;
            }
            ...
            instance.on("error", error => emitter.emit("error", `${colors_1.default.red("@error:")} FFmpeg encountered an error: ${error.message}`));
