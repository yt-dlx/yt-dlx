 ...
 *
 */
function AudioLowest({ query, output, useTor, stream, filter, metadata, verbose }) {
    ...
    (async () => {
        try {
            ...
            if (!engineData.AudioLowF?.url) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Lowest quality audio URL was not found.`);
                return;
            }
