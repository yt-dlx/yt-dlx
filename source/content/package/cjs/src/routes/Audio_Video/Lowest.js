...
exports.default = AudioVideoLowest;
 ...
 *
 */
function AudioVideoLowest({ query, stream, verbose, output, metadata, useTor, filter }) {
    ...
    (async () => {
        try {
            ...
            if (!engineData.AudioLowF?.url) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Lowest quality audio URL not found.`);
                return;
            }
