...
var ZodSchema = zod_1.z.object({
    ...
    resolution: zod_1.z.enum(["144p", "240p", "360p", "480p", "720p", "1080p", "1440p", "2160p", "3072p", "4320p", "6480p", "8640p", "12000p"]),
});
/**
 ...
 *
 */
function AudioVideoCustom({ query, stream, output, useTor, filter, metadata, verbose, resolution }) {
    ...
    (async () => {
        try {
            ...
            if (!resolution) {
                emitter.emit("error", `${colors_1.default.red("@error:")} The 'resolution' parameter is always required.`);
                return;
            }
            if (metadata) {
                if (stream) {
            ...
            const vdata = engineData.ManifestHigh?.find((i) => i.format?.includes(resolutionWithoutP));
            if (vdata) {
                if (!vdata.url) {
                    emitter.emit("error", `${colors_1.default.red("@error:")} Video URL not found for resolution: ${resolution}.`);
                    return;
                }
                instance.addInput(vdata.url.toString());
            }
            else {
                emitter.emit("error", `${colors_1.default.red("@error:")} No video data found for resolution: ${resolution}. Use list_formats() maybe?`);
                return;
            }
