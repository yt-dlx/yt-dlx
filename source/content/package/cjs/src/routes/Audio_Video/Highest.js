    ...
    (async () => {
        try {
            ...
            if (!engineData.metaData.thumbnail) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Thumbnail URL not found.`);
                return;
            }
            instance.addInput(engineData.metaData.thumbnail);
            instance.withOutputFormat("avi");
            if (!engineData.AudioHighF?.url) {
                emitter.emit("error", `${colors_1.default.red("@error:")} Highest quality audio URL not found.`);
                return;
            }
