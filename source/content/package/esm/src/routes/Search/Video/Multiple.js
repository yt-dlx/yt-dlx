 ...
 *
 */
export default function search_videos({ query, minViews, maxViews, orderBy, verbose }) {
    ...
    (async () => {
        try {
            ...
            let videos = searchResults.items.map((item) => ({
                id: item.id,
                title: item.title,
                isLive: item.isLive,
            ...
            if (maxViews !== undefined)
                videos = videos.filter(video => video.viewCount <= maxViews);
            if (orderBy) {
