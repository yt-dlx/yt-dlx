...
/**
 * @shortdesc Downloads or streams the highest quality audio from YouTube, with optional video filters applied.
 *
 * @description This function allows you to download or stream the highest available audio quality from YouTube based on a search query or video URL. It automatically selects the highest quality audio format. Although it's primarily an audio function, it provides an option to apply various *video* filters during processing. Additional customization options include saving the output to a specified directory, using Tor for anonymity, enabling verbose logging, streaming the output, or simply fetching the metadata.
 *
 ...
 * - **filter:** An optional string specifying a *video* filter to apply during processing. This parameter is ignored when `metadata` is true. Available video filters include: "invert", "rotate90", "rotate270", "grayscale", "rotate180", "flipVertical", "flipHorizontal".
 *
 ...
 * - `"metadata"`: Emitted only when the `metadata` parameter is true. Provides an object containing comprehensive metadata, including video and audio format details, and a suggested filename.
 ...
 *
 * @example
 * // 5. Download the highest quality audio and apply a video filter
 ...
 *
 * @example
 * // 8. Download the highest quality audio with custom output and apply a video filter
 * YouTubeDLX.Audio.Highest({ query: "your search query or url", output: "./filtered_audio", filter: "rotate90" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 9. Stream the highest quality audio and apply a video filter
 * YouTubeDLX.Audio.Highest({ query: "your search query or url", stream: true, filter: "flipHorizontal" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("stream", (data) => {
 * console.log("Stream available:", data.filename);
 ...
 * YouTubeDLX.Audio.Highest({ query: "your search query or url", output: "./full_options", useTor: true, verbose: true, filter: "invert" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 ...
 * YouTubeDLX.Audio.Highest({ query: "your search query or url", stream: true, useTor: true, verbose: true, filter: "rotate180" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("stream", (data) => {
 * console.log("Stream available:", data.filename);
 ...
 * YouTubeDLX.Audio.Highest({ query: "your search query or url", metadata: true, filter: "grayscale" })
 ...
 *
 */
export default function AudioHighest({ query, output, useTor, stream, filter, metadata, verbose }) {
    ...
    (async () => {
        try {
            ...
            if (error instanceof ZodError)
                emitter.emit("error", `${colors.red("@error:")} Argument validation failed: ${error.errors.map(e => `${e.path.join(".")}: ${e.message}`).join(", ")}`);
            else if (error instanceof Error)
                emitter.emit("error", `${colors.red("@error:")} ${error?.message}`);
            else
                emitter.emit("error", `${colors.red("@error:")} An unexpected error occurred: ${String(error)}`);
        }
        finally {
