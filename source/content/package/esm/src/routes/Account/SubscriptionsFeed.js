 ...
 *
 */
export default function subscriptions_feed(options) {
    ...
    (async () => {
        try {
            ...
            if (verbose)
                console.log(colors.green("@info:"), "Fetching subscriptions feed...");
            if (!cookies) {
            ...
            const contents = feed.contents?.map(sanitizeContentItem) || [];
            const result = { status: "success", data: { contents } };
            if (verbose)
                console.log(colors.green("@info:"), "Subscriptions feed fetched!");
