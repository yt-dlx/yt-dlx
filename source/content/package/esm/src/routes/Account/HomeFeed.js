 ...
 *
 */
export default function home_feed(options) {
    ...
    (async () => {
        try {
            ...
            if (verbose)
                console.log(colors.green("@info:"), "Fetching home feed...");
            if (!cookies) {
                emitter.emit("error", `${colors.red("@error:")} cookies not provided!`);
                return;
            }
            const client = await TubeLogin(cookies);
            if (!client) {
            ...
            homeFeed.contents?.contents?.forEach((section) => {
                if (section?.type === "RichItem" && section?.content?.type === "Video") {
                    ...
                    section.content.contents?.forEach((item) => {
            ...
            if (verbose)
                console.log(colors.green("@info:"), "Home feed fetched!");
