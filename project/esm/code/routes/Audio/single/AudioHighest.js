 ...
 * @returns A Promise that resolves with either `void`, an object containing the `ffmpeg` instance and the output filename, or an object containing the filename and extracted engine data.
 */
export default async function AudioHighest({ query, output, useTor, stream, filter, metadata, verbose, }) {
    try {
        ZodSchema.parse({
            query,
            output,
        ...
        var logProgress = ({ percent, timemark, }) => {
            ...
            process.stdout.write(`\r${color("@prog:")} ${progb} ${color("| @percent:")} ${percent.toFixed(2)}% ${color("| @timemark:")} ${timemark} ${color("| @eta:")} ${formatTime(calculateETA(startTime, percent))}`);
        };
        ff.on("error", (error) => {
