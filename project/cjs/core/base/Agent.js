...
const retor_1 = __importDefault(require("./retor"));
...
async function Agent({ query, verbose, onionTor, }) {
    ...
    let ipAddress;
    if (onionTor) {
        const { sysip, torip } = await (0, retor_1.default)();
        if (!torip)
            ipAddress = sysip;
        else
            ipAddress = torip;
        console.log(colors_1.default.green("@info:"), "system", colors_1.default.green("ipAddress"), sysip);
        console.log(colors_1.default.green("@info:"), "socks5", colors_1.default.green("ipAddress"), torip);
    }
