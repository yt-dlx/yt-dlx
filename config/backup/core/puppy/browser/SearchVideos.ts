  ...
  const QuerySchema = z.object({
    query: z
      ...
      .refine(
        ...
        {
          message:
            "Query must not be a YouTube video/Playlist link",
        },
      ),
      ...
      videoElements.each(async (_: any, vide: any) => {
        const videoId: any = await YouTubeId(
          "https://www.youtube.com" +
            $(vide).find("a").attr("href"),
        );
        const authorContainer = $(vide).find(
          ".ytd-channel-name a",
        );
        ...
        metaTube.push({
          title:
            $(vide).find("#video-title").text().trim() ||
            undefined,
          views:
            $(vide)
              .find(
          ...
          author:
            authorContainer.text().trim() || undefined,
          videoId,
          uploadOn:
          ...
          authorUrl:
            "https://www.youtube.com" +
              authorContainer.attr("href") || undefined,
          videoLink:
    ...
    case "playlist":
      ...
      playlistElements.each((_index: any, element: any) => {
        ...
        playlistMeta.push({
          ...
          playlistLink:
            "https://www.youtube.com" + playlistLink,
          authorUrl: $(element)
            .find(
              ".yt-simple-endpoint.style-scope.yt-formatted-string",
            )
