...
if (pyCheck.error || pyCheck.status !== 0) {
    if (!existsSync(tarballFile)) {
        ...
        https
            .get(pythonTarballURL, response => {
                ...
                file.on("finish", () => {
                    ...
                    spawnSync("rm", ["-rf", tarballFile], { stdio: "inherit" });
                });
            })
            .on("error", err => {
                console.error(
        ...
        console.log(
            color.cyan("Python tarball already present. Skipping download."),
        );
    }
