...
console.log(
  color.cyan("Checking if Python is already installed"),
);
...
if (pyCheck.error || pyCheck.status !== 0) {
  if (!existsSync(tarballFile)) {
    ...
    console.log(
      color.green("Python not found. Downloading..."),
    );
    ...
    https
      .get(pythonTarballURL, response => {
        ...
        file.on("finish", () => {
          ...
          if (
            ...
            console.error(
              color.red("Error extracting Python tarball."),
            );
            process.exit(1);
          }
          ...
          console.log(
            color.green(
              `Changing directory to ${pythonSourceDir}`,
            ),
          );
          ...
          console.log(
            color.green("Running Python --version"),
          );
          ...
          spawnSync(
            "rm",
            ["-rf", pythonSourceDir, tarballFile],
            {
              stdio: "inherit",
            },
          );
      ...
      .on("error", err => {
        console.error(
          color.red(
            `Error downloading Python source tarball: ${err.message}`,
          ),
        );
      });
  } else {
    console.log(
      color.cyan(
        "Python tarball already present. Skipping download.",
      ),
    );
  }
} else
  console.log(
    color.cyan("Python is already installed. Exiting."),
  );
