...
(async () => {
  try {
    for (const filter of videoFilters) {
      YouTube.VideoOnly.Highest({ filter, stream: false, verbose: true, useTor: false, output: "public/video", query: "video-NAME/ID/URL" });
    }
  } catch (error) {
...
(async () => {
  try {
    const result = YouTube.VideoOnly.Highest({
      stream: true,
      verbose: true,
      useTor: false,
      output: "public/video",
  ...
  try {
    ...
    server.get("/video/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.VideoOnly.Highest({ stream: true, verbose: true, useTor: false, query: queryParam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.VideoOnly.Highest({ metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
