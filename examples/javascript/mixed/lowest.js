...
(async () => {
  try {
    for (const filter of audioFilters) {
      YouTube.default.AudioVideo.Lowest({
        audioFilter: filter,
        stream: false,
        verbose: true,
        useTor: false,
...
(async () => {
  try {
    const result = YouTube.default.AudioVideo.Lowest({
      stream: true,
      verbose: true,
      useTor: false,
      output: "public/mix",
  ...
  try {
    ...
    server.get("/mix/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.default.AudioVideo.Lowest({ stream: true, verbose: true, useTor: false, query: queryParam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.default.AudioVideo.Lowest({ metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
