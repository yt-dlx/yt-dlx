...
(async () => {
  try {
    for (const resolution of resolutions) {
      const result = YouTube.VideoOnly.Custom({
        resolution,
        stream: true,
        verbose: true,
        useTor: false,
  ...
  try {
    ...
    server.get("/video/:resolution/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.VideoOnly.Custom({ stream: true, verbose: true, useTor: false, query: queryParam, resolution: resparam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.VideoOnly.Custom({ resolution: "1080p", metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
