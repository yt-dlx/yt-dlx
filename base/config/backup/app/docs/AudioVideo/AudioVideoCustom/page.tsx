    ...
    <div className="max-w-screen-2xl px-6 py-16 mx-auto space-y-12">
     <article className="space-y-8">
      ...
      <p className="text-white/80">
       Uncover an unparalleled solution for effortless audio and video downloads powered by YT-DLX - An advanced <span className="text-red-600">(command-line + Node.js + Streaming)</span> tool
       meticulously designed for avid enthusiasts. YT-DLX stands out as a feature-rich advanced package built upon the foundation of <span className="text-red-600">(Youtube-DL & Python yt-dlx)</span>,
   ...
   <section className="flex flex-col items-center justify-center">
    <div className="max-w-screen-4xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
     <div className="max-w-screen-2xl">
      <h2 className="text-3xl font-bold sm:text-4xl text-red-600">Viewing YtDlx.AudioVideo.Single.Custom()</h2>
      <p className="mt-4 text-white/80">
       yt-dlx accommodates various node.js coding flavours! <span className="text-red-600">(typescript), (commonjs),</span> and <span className="text-red-600">(esm)</span>, ensuring 100% compatibility
       and comprehensive type safety coverage.
      </p>
      <ul className="list-disc m-4 bg-neutral-800/40 shadow-black shadow-2xl p-8 rounded-3xl border border-dashed border-red-600">
       <li>Downloads audio and video from a YouTube video URL with customizable options such as resolution and filters.</li>
       <li>@param query - The YouTube video URL or ID or name.</li>
       <li>@param resolution - The desired resolution for the video. options: 144p, 240p, 360p, 480p, 720p, 1080p, 1440p, 2160p, 3072p, 4320p, 6480p, 8640p, 12000p.</li>
       ...
       <li>param filter - (optional) The video filter to apply. Available options: invert, rotate90, rotate270, grayscale, rotate180, flipVertical, flipHorizontal.</li>
       <li>@param onionTor - (optional) Whether to use Tor for the download or not.</li>
       <li>@returns A Promise that resolves when the audio and video processing is complete. If `stream` is true, it returns an object with the `ffmpeg` command and the `filename`.</li>
      </ul>
      <AnimatePresence>
       {dbAcc.map((item, index) => (
        <motion.div
         ...
         <div onClick={() => handleSet(index)} className="collapse-title text-xl flex items-center justify-center cursor-pointer hover:text-red-600">
          {item.title}
         </div>
