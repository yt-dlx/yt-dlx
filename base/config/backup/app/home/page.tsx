   ...
   <section className="relative flex flex-col items-center justify-center h-screen py-20 p-2">
    ...
    <div className="relative items-center w-full px-5 pb-12 mx-auto">
     <div className="relative h-full flex flex-col items-center justify-center text-center text-[#C4C4C4]">
      <Link href="/docs" className="flex items-center justify-center px-4 -mb-14 text-sm gap-2 p-1 italic text-pink-600 bg-red-900/20 rounded-xl">
       powered by
      ...
      <p className="max-w-6xl -mt-10 mx-auto text-sm xl:text-lg">
       Discover the limitless rhythm of Mixly: your go-to platform for free, unlimited streaming and downloading of music and videos. Immerse yourself in a diverse collection of tunes, from classics
       to the latest hits. Elevate your experience by syncing playlists and playing music together with multiple listeners. Mixly is not just a service; it&apos;s a shared symphony of sound. Enjoy the
       freedom to explore, connect, and create unforgettable moments—all at your fingertips. Mixly, where the beat never stops, and the experience is always in tune with your passion for music and
       videos.
      </p>
      <form
       onSubmit={event => {
      ...
      {TubeSearch.map((item: any, index: number) => (
       <div
        ...
        className={`p-0.5 rounded-xl border ${
         Tube && Tube.TubeUrl === `https://www.youtube.com/watch?v=${item.id}` ? "border-pink-600 shadow-pink-600" : "border-pink-600/30 hover:shadow-pink-600"
        }  shadow-2xl shadow-black hover:border-pink-600`}>
        <Image width={400} height={400} alt="500x500" src={item.thumbnails[0].url} className="object-cover objecjt-center w-full lg:h-48 md:h-36 rounded-t-xl" />
        {getTube.isPending ? (
         ...
         <>
          {Tube && Tube.TubeUrl === `https://www.youtube.com/watch?v=${item.id}` ? (
           <div className="flex flex-col">
            <Link href="/docs" className="flex items-center justify-center p-2 bg-pink-700/50 hover:bg-pink-700/60 text-white/90 mt-0.5 w-full text-sm gap-1 rounded">
             powered by <TbDiamondFilled size={20} /> yt-dlx{" "}
            </Link>
            <audio controls autoPlay autoFocus id="AudioStreamer" src={`/api/saudio?url=${encodeURIComponent(Tube.EnResp)}`} className="w-full bg-[#18181b] shadow-2xl shadow-pink-600" />
           </div>
          ) : (
           <div className="flex flex-col">
            <button
