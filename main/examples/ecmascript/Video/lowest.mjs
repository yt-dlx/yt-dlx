...
(async () => {
  try {
    for (const filter of videoFilters) {
      YouTube.VideoOnly.Lowest({ filter, stream: false, verbose: true, useTor: false, output: "public/video", query: "video-NAME/ID/URL" });
    }
  } catch (error) {
...
(async () => {
  try {
    const result = YouTube.VideoOnly.Lowest({ stream: true, verbose: true, useTor: false, output: "public/video", query: "video-NAME/ID/URL" });
  ...
  try {
    ...
    server.get("/video/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.VideoOnly.Lowest({ stream: true, verbose: true, useTor: false, query: queryParam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.VideoOnly.Lowest({ metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
      ...
      console.log(colors.green("Available Formats:"), { standard: result.VideoLowF });
    }
  } catch (error) {
    console.error(colors.red(error.message));
  }
