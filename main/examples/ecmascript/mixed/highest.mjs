...
(async () => {
  try {
    const result = YouTube.AudioVideo.Highest({ stream: true, verbose: true, useTor: false, output: "public/mix", query: "video-NAME/ID/URL" });
  ...
  try {
    ...
    server.get("/mix/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.AudioVideo.Highest({ stream: true, verbose: true, useTor: false, query: queryParam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.AudioVideo.Highest({ metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
