...
(async () => {
  try {
    for (const filter of audioFilters) {
      YouTube.AudioOnly.Lowest({ filter, stream: false, verbose: true, useTor: false, output: "public/audio", query: "video-NAME/ID/URL" });
    }
  } catch (error) {
...
(async () => {
  try {
    const result = YouTube.AudioOnly.Lowest({ stream: true, verbose: true, useTor: false, output: "public/audio", query: "video-NAME/ID/URL" });
  ...
  try {
    ...
    server.get("/audio/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.AudioOnly.Lowest({ stream: true, verbose: true, useTor: false, query: queryParam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.AudioOnly.Lowest({ metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
