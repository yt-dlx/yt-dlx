...
(async () => {
  try {
    for (const resolution of resolutions) {
      const result = YouTube.AudioOnly.Custom({
        resolution,
        stream: false,
        verbose: true,
        useTor: false,
  ...
  try {
    ...
    server.get("/audio/:resolution/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.AudioOnly.Custom({ stream: true, verbose: true, useTor: false, query: queryParam, resolution: resparam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.AudioOnly.Custom({ resolution: "high", metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
