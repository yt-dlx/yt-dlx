...
(async () => {
  try {
    for (const filter of audioFilters) {
      YouTube.default.AudioOnly.Highest({ filter, stream: false, verbose: true, useTor: false, output: "public/audio", query: "video-NAME/ID/URL" });
    }
  } catch (error) {
...
(async () => {
  try {
    const result = YouTube.default.AudioOnly.Highest({
      stream: true,
      verbose: true,
      useTor: false,
      output: "public/audio",
  ...
  try {
    ...
    server.get("/audio/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.default.AudioOnly.Highest({ stream: true, verbose: true, useTor: false, query: queryParam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.default.AudioOnly.Highest({ metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
