  ...
  try {
    for (const resolution of resolutions) {
      for (const filter of audioFilters) {
        YouTube.default.AudioVideo.Custom({
          resolution,
          audioFilter: filter,
          stream: false,
          verbose: true,
...
(async () => {
  try {
    for (const resolution of resolutions) {
      const result = YouTube.default.AudioVideo.Custom({
        resolution,
        stream: true,
        verbose: true,
        useTor: false,
  ...
  try {
    ...
    server.get("/mix/:resolution/:query", async (req, res) => {
      try {
        ...
        const result = YouTube.default.AudioVideo.Custom({ stream: true, verbose: true, useTor: false, query: queryParam, resolution: resparam });
        if (result && "ffmpeg" in result && "filename" in result) {
...
(async () => {
  try {
    const result = YouTube.default.AudioVideo.Custom({ resolution: "1080p", metadata: true, verbose: true, query: "video-NAME/ID/URL" });
    if (result?.metaData) {
