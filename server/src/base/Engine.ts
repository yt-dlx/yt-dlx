...
// const stats = await fs.promises.stat(cpath);
// if (stats.isDirectory()) {
// const fcpath = await fcprobe(cpath, cprobe);
// if (fcpath) return fcpath;
// } else if (file === cprobe) return cpath;
// }
// return null;
// }
...
// const cprobe = "cprobe.exe";
// const cpath = await fcprobe(process.cwd(), cprobe);
...
export default async function Engine({
  ...
  const VideoLow: any = {};
  const VideoHigh: any = {};
  const ManifestLow: any = {};
  const ManifestHigh: any = {};
  const AudioLowDRC: any = {};
  const AudioHighDRC: any = {};
  const VideoLowHDR: any = {};
  const VideoHighHDR: any = {};
