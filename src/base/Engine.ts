...
export default async function Engine({
    ...
    function propfilter(formats: any[]) {
        return formats.filter(i => {
            return (
                !i.format_note.includes("DRC") && !i.format_note.includes("HDR")
            );
        });
    }
    var payLoad: EngineOutput = {
        ...
        AudioLow: propfilter(Object.values(AudioLow)).map((i: any) =>
            pAudio(i),
        ),
        AudioHigh: propfilter(Object.values(AudioHigh)).map((i: any) =>
            pAudio(i),
        ),
        VideoLowHDR: Object.values(VideoLowHDR).map((i: any) => pVideo(i)),
        VideoHighHDR: Object.values(VideoHighHDR).map((i: any) => pVideo(i)),
        VideoLow: propfilter(Object.values(VideoLow)).map((i: any) =>
            pVideo(i),
        ),
        VideoHigh: propfilter(Object.values(VideoHigh)).map((i: any) =>
            pVideo(i),
        ),
