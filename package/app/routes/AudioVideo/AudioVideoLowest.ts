...
/**
 * Defines the schema for the input parameters used in the `AudioVideoLowest` function.
 *
 * @typedef {object} AudioVideoLowestOptions
...
/**
 * Processes a YouTube video query, applies the lowest quality audio and video format to it with optional filters,
 * and either streams or saves the result.
 *
 * @function AudioVideoLowest
 * @param {AudioVideoLowestOptions} options - The options object containing query and settings.
 * @returns {EventEmitter} The event emitter to handle progress, error, start, end, and stream events.
 *
 * @example
 * const emitter = AudioVideoLowest({ query: "Funny Video", resolution: "720p", filter: "grayscale", stream: true });
