 ...
 *
 * @example
 * const emitter = AudioVideoHighest({ query: "Funny Video", resolution: "1080p", filter: "grayscale", stream: true });
 * emitter.on("progress", progress => console.log(progress));
 */
export default function AudioVideoHighest({ query, stream, verbose, metadata, output, useTor, filter, resolution }: z.infer<typeof ZodSchema>): EventEmitter {
  const emitter = new EventEmitter();

  (async () => {
    try {
      ZodSchema.parse({ query, stream, verbose, metadata, output, useTor, filter, resolution });
