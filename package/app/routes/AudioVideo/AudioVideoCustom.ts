...
/**
 * Defines the schema for the input parameters used in the `AudioVideoCustom` function.
 *
 * @typedef {object} AudioVideoCustomOptions
 ...
 * @property {string} resolution - The resolution of the video (e.g., "1080p", "720p").
 */
 ...
 * @param {AudioVideoCustomOptions} options - The options object containing query and settings.
 * @returns {EventEmitter} The event emitter to handle progress, error, start, end, and stream events.
 *
 * @example
 * const emitter = AudioVideoCustom({ query: "Funny Video", resolution: "1080p", filter: "grayscale", stream: true });
...
export default function AudioVideoCustom({ query, stream, output, useTor, filter, metadata, verbose, resolution }: z.infer<typeof ZodSchema>): EventEmitter {
  ...
  (async () => {
    try {
      ...
      if (vdata) instance.addInput(vdata.url.toString());
