 ...
 *
 * @typedef {object} VideoLowestOptions
...
/**
 * Processes a YouTube video query, applies the lowest quality video format to it with optional filters,
 * and either streams or saves the result.
 *
 * @function VideoLowest
 * @param {VideoLowestOptions} options - The options object containing query and settings.
 * @returns {EventEmitter} The event emitter to handle progress, error, start, end, and stream events.
 *
 * @example
 * const emitter = VideoLowest({ query: "Funny Video", resolution: "480p", filter: "grayscale", stream: true });
