 ...
 * YouTubeDLX.Audio.Highest({ query: "Song title", filter: "bassboost" })
 ...
 * YouTubeDLX.Audio.Highest({ query: "Song title", filter: "bassboost", verbose: true })
 *   .on("data", (audioData) => console.log("Audio data:", audioData))
 *   .on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 3: Download and process highest quality audio with query, filter, and custom output folder
 * YouTubeDLX.Audio.Highest({ query: "Song title", filter: "bassboost", output: "/path/to/folder" })
 *   .on("data", (audioData) => console.log("Audio data:", audioData))
 *   .on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 4: Stream highest quality audio with query and stream enabled
 * YouTubeDLX.Audio.Highest({ query: "Song title", stream: true })
 ...
 * YouTubeDLX.Audio.Highest({ query: "Song title", filter: "bassboost", metadata: true })
 ...
 * YouTubeDLX.Audio.Highest({ query: "Song title", filter: "bassboost", stream: true, metadata: true })
 *   .on("data", (audioData) => console.log("Audio data:", audioData))
 *   .on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 7: Download and process highest quality audio with all parameters (query, output, filter, stream, verbose, metadata)
 * YouTubeDLX.Audio.Highest({ query: "Song title", output: "/path/to/folder", filter: "bassboost", stream: true, verbose: true, metadata: true })
 *   .on("data", (audioData) => console.log("Audio data:", audioData))
 *   .on("error", (err) => console.error("Error:", err));
 */
