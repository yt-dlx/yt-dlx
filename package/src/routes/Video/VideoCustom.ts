 ...
 * VideoCustom({ query: "Node.js tutorial", resolution: "720p" }).on("data", (videoData) => console.log("Video data:", videoData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 2: Process a video with the query, resolution, and a filter (grayscale)
 * VideoCustom({ query: "Node.js tutorial", resolution: "1080p", filter: "grayscale" }).on("data", (videoData) => console.log("Video data:", videoData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 3: Stream a video with the query, resolution, and stream option enabled
 * VideoCustom({ query: "Node.js tutorial", resolution: "480p", stream: true }).on("stream", (streamData) => console.log("Streaming video:", streamData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 4: Process a video with verbose output enabled
 * VideoCustom({ query: "Node.js tutorial", resolution: "720p", verbose: true }).on("data", (videoData) => console.log("Video data:", videoData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 5: Fetch metadata instead of processing the video
 * VideoCustom({ query: "Node.js tutorial", resolution: "1080p", metadata: true }).on("metadata", (metadata) => console.log("Video metadata:", metadata)).on("error", (err) => console.error("Error:", err));
 */
