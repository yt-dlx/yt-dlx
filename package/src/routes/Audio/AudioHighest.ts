 ...
 * AudioHighest({ query: "Song title", filter: "bassboost" }).on("data", (audioData) => console.log("Audio data:", audioData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 2: Download and process highest quality audio with query, filter, and verbose output enabled
 * AudioHighest({ query: "Song title", filter: "bassboost", verbose: true }).on("data", (audioData) => console.log("Audio data:", audioData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 3: Download and process highest quality audio with query, filter, and custom output folder
 * AudioHighest({ query: "Song title", filter: "bassboost", output: "/path/to/folder" }).on("data", (audioData) => console.log("Audio data:", audioData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 4: Stream highest quality audio with query and stream enabled
 * AudioHighest({ query: "Song title", stream: true }).on("stream", (streamData) => console.log("Streaming audio:", streamData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 5: Download and process highest quality audio with query, filter, and metadata output enabled
 * AudioHighest({ query: "Song title", filter: "bassboost", metadata: true }).on("metadata", (metadata) => console.log("Metadata:", metadata)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 6: Download and process highest quality audio with query, filter, stream, and metadata
 * AudioHighest({ query: "Song title", filter: "bassboost", stream: true, metadata: true }).on("data", (audioData) => console.log("Audio data:", audioData)).on("error", (err) => console.error("Error:", err));
 *
 * @example
 * // Example 7: Download and process highest quality audio with all parameters (query, output, filter, stream, verbose, metadata)
 * AudioHighest({ query: "Song title", output: "/path/to/folder", filter: "bassboost", stream: true, verbose: true, metadata: true }).on("data", (audioData) => console.log("Audio data:", audioData)).on("error", (err) => console.error("Error:", err));
 */
